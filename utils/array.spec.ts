...
describe('Any array item matches predicate', () => {
  it('when match is found', () => {
    expect(anyMatching([1, 2, 3, 4, 5, 6], item => item % 2 === 0)).toBe(true);
  });

  it('when match is not found', () => {
    expect(anyMatching([1, 3, 5, 7, 9, 11], item => item % 2 === 0)).toBe(false);
  });
});

describe('All array item matches predicate', () => {
  it('when all match', () => {
    expect(allMatching([2, 4, 6, 8, 10, 12], item => item % 2 === 0)).toBe(true);
  });

  it('when one item does not match', () => {
    expect(allMatching([2, 4, 6, 7, 10, 12], item => item % 2 === 0)).toBe(false);
  });
});
